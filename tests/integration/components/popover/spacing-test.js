...
function assertSpacing(assert, context, expectedSpacing) {
  ...
  assert.ok(expectedSpacing - 2 < actualSpacing && actualSpacing < expectedSpacing + 2,
    `Popover should be ${expectedSpacing}px from the target`);

}

moduleForComponent('popover-on-element', 'Integration | Option | spacing', {
  integration: true
});
...
test('It shows with showOn', function(assert) {
  ...
  this.render(hbs`{{popover-on-element spacing=20}}`);
